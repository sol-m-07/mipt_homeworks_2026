...
│   └── llm_client.py         # клиент openai (в т.ч. streaming)
├── config/
│   └── config.yaml.example   # шаблон настроек (без секретов)
├── tests/                    # unit-тесты
├── htmlcov/                  # отчёт покрытия (после pytest)
├── main.py                   # точка входа
├── pytest.ini
├── .coveragerc
└── README.md
...
**Streaming** — команда `/stream`, реализация в `app/llm_client.py` + `app/display.py`.

**Тесты** (покрытие ≥ 50%):

```bash
cd final_project
uv run pytest tests --cov=app --cov-report=html --cov-report=term --cov-config=.coveragerc
```
...
Запускайте `python main.py` и тесты из каталога `final_project/`.
